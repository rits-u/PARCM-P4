...
Mesh::Mesh(const wchar_t* fullPath) : Resource(fullPath) {
	...
	//std::string pathStr(fullPath ? WStringToString(fullPath) : "");
	/*std::ifstream fileStream(pathStr, std::ios::binary);
	if (!fileStream)
		throw std::exception("failed to open OBJ file");*/

	loadObjFromStream(WStringToString(fullPath));
}

Mesh::Mesh(const void* objData, size_t objSize)
	: Resource(L"")
{
	std::string objStr((const char)objData, objSize);
	//std::istringstream objStream(objStr);
	loadObjFromStream(objStr);
}

void Mesh::loadObjFromStream(std::string objData)
{
	//std::cout << "mesh start" << std::endl;
	tinyobj::attrib_t attributes;
	std::vector<tinyobj::shape_t> shapes;
	...
	bool res = tinyobj::LoadObj(&attributes, &shapes, &materials, &warning, &error, objData.c_str());
	//std::cout << "loaded obj" << std::endl;
